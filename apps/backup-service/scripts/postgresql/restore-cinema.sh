...
RESTORE_DIR="$(postgresql_restore_dir)/cinema-service/${BACKUP_ID}"

postgresql_validate_file_exists "$BACKUP_FILE"
mkdir -p "$RESTORE_DIR"
trap 'rm -rf "$RESTORE_DIR"' EXIT

postgresql_log "[cinema-service] Starting restore"
postgresql_log "[cinema-service] Backup file: ${BACKUP_FILE}"
...
postgresql_log "[cinema-service] Restore completed"
