...
DATABASE_URL="$(postgresql_resolve_value MOVIE_SERVICE_DATABASE_URL "${DATABASE_URL:-}")"
BACKUP_ID="$(postgresql_resolve_value BACKUP_ID "")"
BACKUP_FILE="$(postgresql_backup_file "$BACKUP_ID")"
RESTORE_DIR="$(postgresql_restore_dir)/movie-service/${BACKUP_ID}"

postgresql_validate_file_exists "$BACKUP_FILE"
mkdir -p "$RESTORE_DIR"
trap 'rm -rf "$RESTORE_DIR"' EXIT

postgresql_log "[movie-service] Starting restore"
postgresql_log "[movie-service] Backup file: ${BACKUP_FILE}"
...
postgresql_log "[movie-service] Restore completed"
