...
function latestRecord() {
  return (
    state.history.find(
      (record) => record.type === 'MANUAL' || record.type === 'SCHEDULED'
    ) ||
    state.history[0] ||
    null
  );
...
function renderRestoreOptions() {
  ...
  for (const record of state.history) {
    if (record.type !== 'MANUAL' && record.type !== 'SCHEDULED') {
      continue;
    }
...
function renderHistory() {
  ...
  for (const record of state.history) {
    ...
    if (record.type === 'MANUAL' || record.type === 'SCHEDULED') {
      ...
      restoreButton.className = 'secondary-button restore-button';
      restoreButton.type = 'button';
      restoreButton.textContent = 'Restore';
      restoreButton.addEventListener('click', () => openRestoreDialog(record));
    ...
    row.innerHTML = `
      ...
      <td></td>
    `;
    row.lastElementChild.replaceWith(restoreCell);
