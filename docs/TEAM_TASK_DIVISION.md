...
- Implement the outbox pattern with the tools already used in MovieHub: **Prisma**, **PostgreSQL JSONB**, **NestJS Schedule**, **NestJS TCP ClientProxy**, and the existing **NotificationService/Nodemailer** adapter.
- Persist side-effect requests in the same database transaction that confirms the booking/payment.
- Process notification delivery asynchronously so VNPay IPN and zero-amount payment confirmation do not wait for email, QR generation, or User Service lookup.
- Keep event delivery auditable through retry count, status, and structured logs.

**Affected Artifacts**:

- `apps/booking-service/prisma/schema.prisma` - `OutboxEvents` model and `OutboxStatus` enum
- `apps/booking-service/prisma/migrations/20260508000100_add_outbox_events/migration.sql` - PostgreSQL migration
- `apps/booking-service/src/app/payment/payment.service.ts` - writes outbox event inside the payment confirmation transaction
- `apps/booking-service/src/app/outbox/outbox.module.ts` - registers worker dependencies and `ScheduleModule`
- `apps/booking-service/src/app/outbox/outbox.service.ts` - polls, claims, dispatches, retries, and marks events
- `apps/booking-service/src/app/notification/notification.service.ts` - sends email through Nodemailer
- `apps/booking-service/src/app/ticket/ticket.service.ts` - generates QR codes before notification delivery
- `apps/booking-service/src/app/app.module.ts` - imports `OutboxModule`

**Concrete Implementation Tasks**:

1. Use the real Prisma schema, not a separate Knex/SQL table name:
   ```prisma
   enum OutboxStatus {
     PENDING
     PROCESSING
   ...
   ```
   - `payload` is JSONB in PostgreSQL and stores only data needed to dispatch the side effect.
   - `@@unique([aggregate_id, event_type])` makes booking confirmation notification idempotent per booking.
   - `@@index([status, created_at])` supports efficient polling of oldest pending events first.

2. Record the outbox event inside the same Prisma transaction that confirms payment/booking:
   ```typescript
   ...
   ```
   - This is the critical guarantee: payment, booking, tickets, promotion usage, and outbox event commit or roll back together.
   - `upsert` is the right Prisma tool here because VNPay IPN can be retried and zero-amount payment can follow a different confirmation path.
   - The synchronous payment path only writes the intent to notify; email/QR/User Service calls happen later in `OutboxService`.

3. Use `OutboxModule` as the worker boundary:
   ```typescript
   @Module({
     imports: [
   ...
   ```
   - `ScheduleModule` is the correct NestJS tool for periodic background work.
   - `NotificationModule` owns email delivery details.
   - `TicketModule` owns QR code generation.
   - `ClientProxy` is used only for optional User Service enrichment, not for the transaction itself.

4. Poll and claim events with the scheduled worker:
   ```typescript
   ...
   ```
   - Batch size is intentionally bounded to avoid one worker cycle monopolizing the Booking Service.
   - A local `processing` flag prevents overlapping polls inside the same process.
   - For multi-replica production, replace the claim step with a row-locking query such as `FOR UPDATE SKIP LOCKED` via Prisma raw SQL, or run exactly one outbox worker replica.

5. Claim each event before dispatch:
   ```typescript
   ...
   ```
   - `updateMany` acts as an atomic compare-and-set: if another worker already claimed it, this worker skips it.
   - This avoids duplicate dispatch in a single-process worker and reduces duplicate risk in normal operation.

6. Dispatch only registered event types:
   ```typescript
   ...
   ```
   - Current implemented event: `booking.confirmed.notification`.
   - Handler loads booking/tickets/concessions from Booking DB, optionally enriches user details through User Service TCP, generates QR via `TicketService`, then delegates email delivery to `NotificationService`.
   - Unknown events are logged instead of crashing the worker.

7. Retry and dead-letter policy:
   - On success: set `status = DELIVERED`, `processed_at = now`, clear `last_error`.
   - On failure: increment `delivery_attempts`, save sanitized `last_error`, and set:
     - `PENDING` when attempts are still below 3.
     - `FAILED` when attempts reach 3.
   - `FAILED` rows are not deleted; they remain queryable for manual replay or support investigation.
   - Operational query:
     ```sql
     SELECT id, aggregate_id, event_type, delivery_attempts, last_error, updated_at
     FROM "OutboxEvents"
     WHERE status = 'FAILED'
     ORDER BY updated_at DESC;
     ```

8. Why this tool choice is correct for MovieHub:
   - **Prisma `$transaction`** is required because Booking Service already uses Prisma models and generated clients.
   - **PostgreSQL table outbox** is required because booking/payment/ticket state is already transactional in PostgreSQL; Redis should not be used as the durable outbox.
   - **NestJS Schedule** is enough for the current project scale and keeps the worker inside Booking Service.
   - **Redis Pub/Sub remains separate**: it is used for real-time seat updates after confirmation, not for durable email delivery.
   - **Nodemailer via `NotificationService`** is the correct provider boundary; outbox should not know SMTP details.

9. Test Scenarios:
   - VNPay success callback writes payment, booking status, ticket status, promotion usage, and outbox row in one transaction.
   - Zero-amount payment path also writes the same `BOOKING_CONFIRMED_NOTIFICATION` outbox event.
   - Duplicate VNPay IPN does not create a duplicate notification event because `aggregate_id + event_type` is unique and `upsert` is used.
   - Worker picks pending event, sends notification, and marks it `DELIVERED`.
   - Notification provider failure increments `delivery_attempts` and retries until the event becomes `FAILED`.
   - Booking confirmation response remains fast even when email or User Service lookup is slow.
