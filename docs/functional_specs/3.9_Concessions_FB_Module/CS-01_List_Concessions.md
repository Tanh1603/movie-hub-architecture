...
Actor -> GW: (1) GET /v1/concessions [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `concession.findAll` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "List Concessions" function and receives the request/event.<br>❖ The system uses trigger [GET /v1/concessions]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [cinemaId], [category], [available].<br>❖ Optional fields: [cinemaId], [category], [available].<br>❖ Field constraint: [cinemaId] is optional, type string, query parameter.<br>❖ Field constraint: [category] is optional, type ConcessionCategory, query parameter.<br>❖ Field constraint: [available] is optional, type string, query parameter.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Retrieval Rules:<br>❖ Concession create requires name, category, and price; category must be ConcessionCategory, price/inventory must be numeric, and inventory updates require numeric quantity.<br>❖ Concession lookups can filter by cinema, category, and availability; create/update normalizes price and inventory values.<br>❖ Unavailable or out-of-stock concessions cannot be used by booking price calculation.<br>❖ Inventory update adjusts the persisted inventory number atomically for the targeted concession.<br>❖ List responses must honor supported filters, pagination defaults, and cinema/user scoping before returning data. |
