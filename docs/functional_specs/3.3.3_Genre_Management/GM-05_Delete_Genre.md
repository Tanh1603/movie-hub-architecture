...
Actor -> GW: (1) DELETE /v1/genres/:id [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `genre.deleted` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Delete Genre" function and receives the request/event.<br>❖ The system uses trigger [DELETE /v1/genres/:id]. |
...
| (5) | BR3 | Deleting Rules:<br>❖ Genre create/update requires non-empty name; payload is strict and unknown fields are not part of the contract.<br>❖ Delete/cancel actions must be blocked when dependent records or invalid terminal states would cause data inconsistency. |
