...
Actor -> GW: (1) GET /v1/bookings/admin/revenue-report [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `booking.getRevenueReport` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Get Revenue Report" function and receives the request/event.<br>❖ The system uses trigger [GET /v1/bookings/admin/revenue-report]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [startDate], [endDate], [cinemaId], [groupBy].<br>❖ The system validates data according to DTO/schema GetRevenueReportDto.<br>❖ Required fields: [startDate], [endDate].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [cinemaId], [groupBy].<br>❖ Field constraint: [startDate] is required, type Date.<br>❖ Field constraint: [endDate] is required, type Date.<br>❖ Field constraint: [cinemaId] is optional, type string.<br>❖ Field constraint: [groupBy] is optional, type 'day' | 'week' | 'month' | 'cinema', allowed values: day, week, month, cinema.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
