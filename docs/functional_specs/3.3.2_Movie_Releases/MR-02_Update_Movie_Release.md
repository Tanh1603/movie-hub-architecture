...
Actor -> GW: (1) PUT /v1/movie-releases/:id [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `movie.release.updated` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Update Movie Release" function and receives the request/event.<br>❖ The system uses trigger [PUT /v1/movie-releases/:id]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [id], [movieId], [startDate], [endDate], [note].<br>❖ The system validates data according to DTO/schema CreateMovieReleaseRequest.<br>❖ Required fields: [id], [movieId], [startDate].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [endDate], [note].<br>❖ Field constraint: [id] is required, type string, path parameter.<br>❖ Field constraint: [movieId] is required, type value, must be a UUID.<br>❖ Field constraint: [startDate] is required, type date, is coerced from request input.<br>❖ Field constraint: [endDate] is optional, type date, is coerced from request input.<br>❖ Field constraint: [note] is optional, type string, maximum 500.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Updating Rules:<br>❖ Movie release create requires UUID movieId and startDate; endDate and note are optional, and note is limited to 500 characters.<br>❖ Update actions must merge only allowed DTO fields and leave omitted fields unchanged. |
