...
Actor -> GW: (1) POST /v1/movie-releases [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `movie.release.created` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Create Movie Release" function and receives the request/event.<br>❖ The system uses trigger [POST /v1/movie-releases]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [movieId], [startDate], [endDate], [note].<br>❖ The system validates data according to DTO/schema CreateMovieReleaseRequest.<br>❖ Required fields: [movieId], [startDate].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [endDate], [note].<br>❖ Field constraint: [movieId] is required, type value, must be a UUID.<br>❖ Field constraint: [startDate] is required, type date, is coerced from request input.<br>❖ Field constraint: [endDate] is optional, type date, is coerced from request input.<br>❖ Field constraint: [note] is optional, type string, maximum 500.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Creating Rules:<br>❖ Movie release create requires UUID movieId and startDate; endDate and note are optional, and note is limited to 500 characters.<br>❖ Create actions must reject duplicate/conflicting records before persistence and return the created DTO after persistence. |
