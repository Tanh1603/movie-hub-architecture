...
Actor -> GW: (1) DELETE /v1/movie-releases/:id [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `movie.release.deleted` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Delete Movie Release" function and receives the request/event.<br>❖ The system uses trigger [DELETE /v1/movie-releases/:id]. |
...
| (5) | BR3 | Deleting Rules:<br>❖ Movie release create requires UUID movieId and startDate; endDate and note are optional, and note is limited to 500 characters.<br>❖ Delete/cancel actions must be blocked when dependent records or invalid terminal states would cause data inconsistency. |
