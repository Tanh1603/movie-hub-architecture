...
Actor -> GW: (1) DELETE /v1/reviews/:id [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `review.deleted` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Delete Review" function and receives the request/event.<br>❖ The system uses trigger [DELETE /v1/reviews/:id]. |
...
| (5) | BR3 | Deleting Rules:<br>❖ Delete/cancel actions must be blocked when dependent records or invalid terminal states would cause data inconsistency. |
