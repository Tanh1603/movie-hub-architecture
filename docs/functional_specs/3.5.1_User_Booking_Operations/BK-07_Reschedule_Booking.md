...
Actor -> GW: (1) POST /v1/bookings/:id/reschedule [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `booking.reschedule` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Reschedule Booking" function and receives the request/event.<br>❖ The system uses trigger [POST /v1/bookings/:id/reschedule]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [id], [newShowtimeId], [reason].<br>❖ The system validates data according to DTO/schema RescheduleBookingDto.<br>❖ Required fields: [id], [newShowtimeId].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [reason].<br>❖ Field constraint: [id] is required, type string, path parameter.<br>❖ Field constraint: [newShowtimeId] is required, type string.<br>❖ Field constraint: [reason] is optional, type string.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Creating Rules:<br>❖ The system executes the main business operation and returns the requested data or persists the state change consistently. |
