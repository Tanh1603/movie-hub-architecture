...
Actor -> GW: (1) POST /v1/showtimes/showtime [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `showtime.create_showtime` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Create Showtime" function and receives the request/event.<br>❖ The system uses trigger [POST /v1/showtimes/showtime]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [movieId], [movieReleaseId], [cinemaId], [hallId], [startTime], [format], [language], [subtitles], [status].<br>❖ The system validates data according to DTO/schema CreateShowtimeRequest.<br>❖ Required fields: [movieId], [movieReleaseId], [cinemaId], [hallId], [startTime], [language].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [format], [subtitles], [status].<br>❖ Default values: [format] = FormatEnum.TWO_D, [subtitles] = [], [status] = ShowtimeStatusEnum.SELLING.<br>❖ Field constraint: [movieId] is required, type string, minimum 1.<br>❖ Field constraint: [movieReleaseId] is required, type string, minimum 1.<br>❖ Field constraint: [cinemaId] is required, type string, minimum 1.<br>❖ Field constraint: [hallId] is required, type string, minimum 1.<br>❖ Field constraint: [format] is optional, type value, default FormatEnum.TWO_D, allowed values: FormatEnum.<br>❖ Field constraint: [language] is required, type value, minimum 1; maximum 10.<br>❖ Field constraint: [subtitles] is optional, type array<string>, default [].<br>❖ Field constraint: [status] is optional, type value, default ShowtimeStatusEnum.SELLING, allowed values: ShowtimeStatusEnum.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Creating Rules:<br>❖ CreateShowtimeRequest requires movieId, movieReleaseId, cinemaId, hallId, startTime in yyyy-MM-dd HH:mm:ss, language, and enum format/status values.<br>❖ Deleting or cancelling showtimes must respect existing bookings/reservations and service constraints.<br>❖ Seat map/TTL reads combine persisted showtime/seat data with Redis held-seat state for the requesting user.<br>❖ Create actions must reject duplicate/conflicting records before persistence and return the created DTO after persistence. |
