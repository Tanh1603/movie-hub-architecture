...
Actor -> GW: (1) GET /v1/cinemas [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `cinema.getAll` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "List All Cinemas" function and receives the request/event.<br>❖ The system uses trigger [GET /v1/cinemas]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [status].<br>❖ Optional fields: [status].<br>❖ Field constraint: [status] is optional, type CinemaStatusEnum, query parameter.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Retrieval Rules:<br>❖ Cinema IDs and status filters must be valid; status defaults to ACTIVE where the controller applies CinemaStatusEnum defaults.<br>❖ Deletes must fail when dependent halls, seats, showtimes, or reservations would violate service constraints.<br>❖ Status filters use the relevant enum and default active status when controller code applies a default.<br>❖ Cinema/Hall/Ticket pricing responses are returned from Cinema Service without exposing internal persistence-only fields.<br>❖ List responses must honor supported filters, pagination defaults, and cinema/user scoping before returning data. |
...
| (8) | BR5 | Error Handling Rules:<br>❖ Gateway forwards the request to the target service through microservice pattern cinema.getAll and propagates service errors through the common exception layer.<br>❖ If a constraint, conflict, or unexpected failure occurs, the system shows error message MSG 9. |
