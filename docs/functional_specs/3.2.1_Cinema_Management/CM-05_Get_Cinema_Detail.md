...
Actor -> GW: (1) GET /v1/cinemas/:id [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `CINEMA.GET_CINEMA_DETAIL` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Get Cinema Detail" function and receives the request/event.<br>❖ The system uses trigger [GET /v1/cinemas/:id]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [lat], [lon], [id].<br>❖ Required fields: [id].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Optional fields: [lat], [lon].<br>❖ Field constraint: [lat] is optional, type string, query parameter.<br>❖ Field constraint: [lon] is optional, type string, query parameter.<br>❖ Field constraint: [id] is required, type string, path parameter.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
...
| (8) | BR5 | Error Handling Rules:<br>❖ Gateway forwards the request to the target service through microservice pattern CINEMA.GET_CINEMA_DETAIL and propagates service errors through the common exception layer.<br>❖ If a constraint, conflict, or unexpected failure occurs, the system shows error message MSG 9. |
