...
Actor -> GW: (1) GET /v1/halls/hall/:hallId [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `hall.get_detail` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Get Hall by ID" function and receives the request/event.<br>❖ The system uses trigger [GET /v1/halls/hall/:hallId]. |
...
| (8) | BR5 | Error Handling Rules:<br>❖ Gateway forwards the request to the target service through microservice pattern hall.get_detail and propagates service errors through the common exception layer.<br>❖ If a constraint, conflict, or unexpected failure occurs, the system shows error message MSG 9. |
