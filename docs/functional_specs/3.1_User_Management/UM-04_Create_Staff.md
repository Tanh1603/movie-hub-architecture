...
Actor -> GW: (1) POST /v1/staffs [BR1]
GW -> GW: (3) Validate authentication, params, query, and body [BR2]
GW -> SVC: (5) Send `staff.created` [BR3]
...
| :--- | :--- | :--- |
| (1) | BR1 | Loading Screen Rules:<br>❖ The system loads the "Create Staff" function and receives the request/event.<br>❖ The system uses trigger [POST /v1/staffs]. |
| (3) | BR2 | Validate Rules:<br>❖ The system checks the items [cinemaId], [fullName], [email], [phone], [gender], [dob], [position], [status], [workType], [shiftType], [salary], [hireDate].<br>❖ The system validates data according to DTO/schema CreateStaffRequest.<br>❖ Required fields: [cinemaId], [fullName], [email], [phone], [gender], [dob], [position], [status], [workType], [shiftType], [salary], [hireDate].<br>❖ If any required entries are empty, the system shows error message MSG 1.<br>❖ Field constraint: [cinemaId] is required, type value, must be a UUID.<br>❖ Field constraint: [fullName] is required, type string, minimum 1.<br>❖ Field constraint: [email] is required, type string, must be an email format.<br>❖ Field constraint: [phone] is required, type string, minimum 9.<br>❖ Field constraint: [gender] is required, type enum, allowed values: Gender.<br>❖ Field constraint: [dob] is required, type date, is coerced from request input.<br>❖ Field constraint: [position] is required, type enum, allowed values: StaffPosition.<br>❖ Field constraint: [status] is required, type enum, allowed values: StaffStatus.<br>❖ Field constraint: [workType] is required, type enum, allowed values: WorkType.<br>❖ Field constraint: [shiftType] is required, type enum, allowed values: ShiftType.<br>❖ Field constraint: [salary] is required, type number, minimum 0.<br>❖ Field constraint: [hireDate] is required, type date, is coerced from request input.<br>❖ If any type, format, enum, range, date, UUID, or email constraint is invalid, the system shows error message MSG 4. |
| (5) | BR3 | Creating Rules:<br>❖ User data is sourced from Clerk-synchronized records and staff context; Clerk webhook payloads must be verified before processing.<br>❖ Staff managers are limited to their assigned cinema for create, view, update, and delete operations where staffContext.cinemaId is present.<br>❖ Create actions must reject duplicate/conflicting records before persistence and return the created DTO after persistence. |
